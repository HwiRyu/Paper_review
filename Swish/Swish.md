...
Relu를 충분히 깊게 쌓는 것으로 임의의 연속함수를 표현할 수 있기 때문에 어떤 연속 활성화함수를 사용하더라도 Relu를 쓰는 것과 큰 차이가 없을 거라는 추측을 했었는데, smooth한 성질이 정말 Relu에 비해 더 좋은 결과를 내는게 맞을까? smooth하기 때문에 더 optimize하기 좋다는게 이해가 잘 되지 않는다. (위 추측이 정말 사실인지는 학습가능한지를 고려해야할 것 같다. 임의의 연속함수를 표현할 수 있는 것과 그 연속함수를 신경망이 학습으로 찾아가는 것은 다른 문제다.)
